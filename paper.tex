...
\author{
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \textbf{Aaryan Saharan} \\
        \textit{Dept. of Computer Science \& Eng.} \\
        \textit{Thapar Inst. of Eng. \& Tech.} \\
        Patiala, Punjab, India \\
        \href{mailto:email1@thapar.edu}{email1@thapar.edu}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \textbf{Aniket Kumar} \\
        ...
        \textit{Thapar Inst. of Eng. \& Tech.} \\
        Patiala, Punjab, India \\
        \href{mailto:email2@thapar.edu}{email2@thapar.edu}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.32\textwidth}
        \centering
        \textbf{Ayush Kumar} \\
        ...
        \textit{Thapar Inst. of Eng. \& Tech.} \\
        Patiala, Punjab, India \\
        \href{mailto:email3@thapar.edu}{email3@thapar.edu}
    \end{minipage}
}
...
Plant diseases inflict over \$220 billion in annual agricultural losses. While automated diagnosis using deep learning offers a viable alternative to scarce expert pathology, existing systems face critical barriers to field deployment: edge hardware constraints, black-box predictions, and severe performance drops outside controlled environments.
...
\begin{itemize}
    \item \textbf{Lightweight Backbone:} A CBAM-augmented MobileNetV3 that matches ResNet-50 accuracy on PlantVillage while reducing parameter footprint by 95.3\% (1.12M vs. 24.04M).
...
\subsection{Primary Dataset: PlantVillage Taxonomy}
The primary controlled laboratory dataset is drawn from the public PlantVillage corpus ($N = 54,305$ RGB images spanning 38 crop-disease taxonomy classes). For evaluation, an 80\%/20\% train-validation split was enforced ($N_{\text{train}} = 43,444$ images, $N_{\text{val}} = 10,861$ images), with a primary held-out evaluation set comprising 33 target crop-disease classes. Represented crop species include apple, blueberry, cherry, corn (maize), grape, orange, peach, bell pepper, potato, raspberry, soybean, squash, strawberry, and tomato. Pathological categories encompass fungal pathogens (e.g., \textit{Venturia inaequalis}, \textit{Alternaria solani}), bacterial infections (e.g., \textit{Xanthomonas} spp.), and viral diseases (e.g., Tomato Yellow Leaf Curl Virus, Tobamovirus), alongside healthy foliar controls for each plant species.

\subsection{Secondary Dataset: Kashmiri Apple Orchard Field Dataset}
To rigorously evaluate out-of-domain (OOD) generalization under real-world agricultural conditions, a disjoint field dataset was acquired: the Kashmiri Apple Field Dataset ($N = 419$ raw farm photographs). Collected across active Kashmiri apple orchards, the dataset contains four target field classes: Apple Scab, Black Rot, Cedar Apple Rust, and Healthy Leaves ($N_{\text{train}} = 335$ images, $N_{\text{val}} = 84$ images). Unlike studio-captured laboratory samples, these photographs incorporate unconstrained farm noise, including direct solar glare, variable shadows, background soil/weed clutter, and complex leaf orientations. This dataset was reserved exclusively for zero-shot testing and real-world field adaptation experiments.
...
For real-world field adaptation, an automated 3-stage preprocessing pipeline was engineered:
\begin{itemize}
...
% --- SYSTEM WORKFLOW TIKZ FLOWCHART ---
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[
    node distance=0.8cm and 0.6cm,
    block/.style={rectangle, draw=blue!80!black, fill=blue!5, thick, inner sep=6pt, rounded corners=3pt, align=center, font=\small},
    proc/.style={rectangle, draw=teal!80!black, fill=teal!10, thick, inner sep=6pt, rounded corners=3pt, align=center, font=\small},
    attn/.style={rectangle, draw=orange!80!black, fill=orange!10, thick, inner sep=6pt, rounded corners=3pt, align=center, font=\small},
    out/.style={rectangle, draw=purple!80!black, fill=purple!10, thick, inner sep=6pt, rounded corners=3pt, align=center, font=\small},
    line/.style={draw, -{Stealth[length=2.5mm]}, thick, color=gray!80!black}
]

% Nodes
\node [block] (input) {\textbf{Unconstrained Field Photo}\\Raw $224 \times 224 \times 3$ RGB};
\node [proc, right=of input] (isolation) {\textbf{Foliar ROI Isolation}\\HSV/Lab Masking\\(Background Soil Removal)};
\node [proc, right=of isolation] (sampling) {\textbf{$8\times$ Multi-Scale Patches}\\$N = 2,680$ Patches\\(+800\% Lesion Density)};
\node [attn, right=of sampling] (cbam) {\textbf{Attention-MobileNetV3}\\CBAM (Channel + Spatial)\\$1.12\text{M Parameters}$};
\node [out, right=of cbam] (deploy) {\textbf{5-Crop TTA Logits}\\Grad-CAM Overlay +\\Energy OOD Rejection\\(\textbf{76.19\% Field Acc})};

% Arrows
\path [line] (input) -- (isolation);
\path [line] (isolation) -- (sampling);
\path [line] (sampling) -- (cbam);
\path [line] (cbam) -- (deploy);

\end{tikzpicture}%
}
\caption{End-to-end system workflow: from raw orchard photo input through foliar leaf ROI isolation, $8\times$ multi-scale patch super-sampling, Attention-MobileNetV3 feature extraction, and 5-crop TTA inference.}
...
% --- TABLE 1: Span Both Columns (table*) + Auto-Fit ---
\begin{table*}[t]
\caption{Comparative validation benchmarks on the primary 38-class PlantVillage dataset under identical training configurations.}
...
\midrule
Total Parameters & 1,083,201 & \textbf{1,124,771} & 24,041,057 \\
...
To recover field classification performance, the engineered 3-stage field adaptation pipeline was applied to Attention-MobileNetV3. Table~\ref{tab:stage_progression} tracks accuracy gains across each progressive engineering milestone, scaling real-world field accuracy from \textbf{11.46\% up to 76.19\%} (+64.73\% overall net gain).

% --- TABLE 2: Span Both Columns (table*) + Auto-Fit ---
\begin{table*}[t]
\caption{Stage-by-stage real-world field optimization progression for Attention-MobileNetV3 on the Kashmiri Apple Orchard dataset.}
...
Stage 3: Foliar Mask + $8\times$ Patches & 75.00\% & 0.7582 & 0.7490 & Background Soil Masking \& 2.6k Patches \\
Stage 4: $8\times$ Patches + 5-Crop TTA & \textbf{76.19\%} & \textbf{0.7668} & \textbf{0.7573} & \textbf{+64.73\% Total Net Accuracy Gain} \\
\bottomrule
\end{tabular}%
}
\end{table*}
...
\begin{table*}[t]
\caption{Comparative 3-model real-world field benchmark on the Kashmiri Apple Orchard dataset ($N = 2,680$ patches).}
...
This study demonstrates that \mbox{Attention-MobileNetV3} successfully resolves the longstanding trade-off between computational efficiency, visual transparency, and out-of-domain field generalization in automated plant disease diagnosis. Incorporating Convolutional Block Attention Modules (CBAM) into a MobileNetV3-Small backbone yields a highly efficient model containing just 1.12~Million parameters ($a \mathbf{95.3\% \text{ reduction}}$ vs. ResNet-50) with sub-10ms GPU latency while achieving 99.81\% classification accuracy on the 38-class PlantVillage benchmark dataset. 

Furthermore, to overcome catastrophic out-of-domain performance degradation when deploying laboratory models to real-world farms, an automated 3-stage field adaptation architecture was engineered: combining \textit{HSV/Lab Foliar ROI Isolation}, \textit{$8\times$ Multi-Scale Patch Super-Sampling} ($N = 2,680$ patches), and \textit{5-Crop Test-Time Augmentation (TTA)}. On the unseen Kashmiri Apple Orchard dataset, this pipeline recovers classification accuracy from an initial zero-shot collapse of 11.46\% up to 76.19\% field accuracy, outperforming standard MobileNetV3 (70.24\%) and heavy ResNet-50 (67.86\%) backbones. Integrated Grad-CAM heatmaps and Energy-based OOD rejection guardrails provide visual transparency and deployment safety, establishing a robust foundation for edge agricultural artificial intelligence.
...
\bibitem{ref10} 
Y.~Miao, W.~Meng, and X.~Zhou, ``SerpensGate-YOLOv8: An enhanced YOLOv8 model for accurate plant disease detection,'' \textit{Frontiers in Plant Science}, vol.~15, art.~1514832, 2025, doi: 10.3389/fpls.2024.1514832.
...
\bibitem{ref13} 
Y.~Peng and Y.~Wang, ``Leaf disease image retrieval with object detection and deep metric learning,'' \textit{Frontiers in Plant Science}, vol.~13, art.~963302, 2022, doi: 10.3390/fpls.13.963302.
